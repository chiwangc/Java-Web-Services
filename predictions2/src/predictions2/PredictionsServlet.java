...
public class PredictionsServlet extends HttpServlet {
	...
	predictions.setServletContext(getServletContext());
    }
    // GET /predictions2
	...
	if (key == null) {
	    ...
	    // Sort the map's value for readability.
    ...
    // POST /prediction2
    // HTTP body should contain two keys, one for the predictor ("who") and another for the prediction ("what").
    @Override
	...
	/* A workaround is necessary for a PUT request because neither Tomcat nor Jetty generates a workable parameter map for this HTTP verb. */
	String key = null;
	String rest = null;
	boolean who = false;
	/* Let the hack begin. */
	try {
	    ...
	    /* To simplify the hack, assume that the PUT request has exactly two parameter: the id and either who or what. Assume, further,
	     * that the id comes first. From the client side, a hash character # separates the id and the who/what, e.g., id=33#who=Homer Allision
	     */
	...
	// Lookup the specified prediction.
